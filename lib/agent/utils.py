...
def load(browsers, use_local_file=True, verify_ssl=True):
    ...
    if use_local_file:
        try:
            json_lines = (
                ilr.files("agent.data").joinpath("browsers.json").read_text()
            )
            ...
            try:
                with open(
                    resource_filename("agent", "data/browsers.json")
                ) as file:
...
from agent import settings  # noqa # isort:skip
from agent.errors import FakeUserAgentError  # noqa # isort:skip
